...
export default function ErpHeader({ onBack }) {
  const { t } = useTranslation('ocd_how_erp_works');

  return (
    <header className="erp-activity__header">
      {/* Left: Subtle Back button */}
      <button
        type="button"
        onClick={onBack}
        className="erp-activity__back-btn"
        aria-label="Go back"
      >
      ...
      {/* Center: Exactly centered OCDMantra Logo */}
      <img
        src={OCDMANTRA_LOGO_URL}
        alt="OCDMantra"
        className="erp-activity__logo"
      />
    </header>
  );
}
