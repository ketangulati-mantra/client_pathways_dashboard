...
async function runTests() {
  ...
  try {
    ...
    console.assert(res1.nextMilestone === 7, `Expected nextMilestone 7, got ${res1.nextMilestone}`);
    console.assert(res1.daysUntilNextMilestone === 6, `Expected daysUntilNextMilestone 6, got ${res1.daysUntilNextMilestone}`);
    console.log('✓ Test 1 Passed: First check-in initializes streak = 1.\n');
    ...
    // Seed 3 consecutive days prior to today
    let d = today;
    const pastDates = [];
    for (let i = 0; i < 4; i++) {
      pastDates.push(d);
    ...
    // Insert 4 consecutive days (today, yesterday, -2d, -3d)
    ...
    console.assert(res8.newMilestoneAchieved === null, `Expected newMilestoneAchieved null on repeat, got ${JSON.stringify(res8.newMilestoneAchieved)}`);
    ...
    console.log(`2026-08-31T02:00:00Z in America/New_York: ${nyDate} (Aug 30)`);
    console.log(`2026-08-31T02:00:00Z in Asia/Tokyo: ${tokyoDate} (Aug 31)`);
